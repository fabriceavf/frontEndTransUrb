...
class UpdateSoldablesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateSoldablesScreen({Key? key ,required this.data}){
try {
...
final UpdateSoldablesState state = Get.put(UpdateSoldablesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Soldables',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateSoldablesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('soldables').where('id','=',id).update(data);
    var allSoldables=await db.table('soldables').get();
    print('allSoldables');
    print(allSoldables);

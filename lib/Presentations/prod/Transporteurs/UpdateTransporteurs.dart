...
class UpdateTransporteursScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTransporteursScreen({Key? key ,required this.data}){
try {
...
final UpdateTransporteursState state = Get.put(UpdateTransporteursState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Transporteurs',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTransporteursState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('transporteurs').where('id','=',id).update(data);
    var allTransporteurs=await db.table('transporteurs').get();
    print('allTransporteurs');
    print(allTransporteurs);

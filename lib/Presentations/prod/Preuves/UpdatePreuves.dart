...
class UpdatePreuvesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdatePreuvesScreen({Key? key ,required this.data}){
try {
...
final UpdatePreuvesState state = Get.put(UpdatePreuvesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Preuves',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdatePreuvesState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['role']?.text=data['role'].toString();
            ...
            this.inputControllers['remarque']?.text=data['remarque'].toString();
    ...
    await db.table('preuves').where('id','=',id).update(data);

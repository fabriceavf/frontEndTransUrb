...
class UpdateHorairestypespostesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateHorairestypespostesScreen({Key? key ,required this.data}){
try {
...
final UpdateHorairestypespostesState state = Get.put(UpdateHorairestypespostesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Horairestypespostes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateHorairestypespostesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('horairestypespostes').where('id','=',id).update(data);

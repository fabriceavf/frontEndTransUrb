...
class UpdateTypesmoyenstransportsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypesmoyenstransportsScreen({Key? key ,required this.data}){
try {
...
final UpdateTypesmoyenstransportsState state = Get.put(UpdateTypesmoyenstransportsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typesmoyenstransports',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypesmoyenstransportsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typesmoyenstransports').where('id','=',id).update(data);
    var allTypesmoyenstransports=await db.table('typesmoyenstransports').get();
    print('allTypesmoyenstransports');
    print(allTypesmoyenstransports);

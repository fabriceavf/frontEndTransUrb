...
class UpdateSexesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateSexesScreen({Key? key ,required this.data}){
try {
...
final UpdateSexesState state = Get.put(UpdateSexesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Sexes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateSexesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('sexes').where('id','=',id).update(data);
    var allSexes=await db.table('sexes').get();
    print('allSexes');
    print(allSexes);

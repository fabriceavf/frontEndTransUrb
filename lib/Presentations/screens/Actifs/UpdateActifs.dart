...
class UpdateActifsScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateActifsScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateActifsState state = Get.put(UpdateActifsState());
    state.setData(data);
...
class UpdateActifsState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('actifs').where('id', '=', id).update(data);
    var allActifs = await db.table('actifs').get();
    print('allActifs');
    print(allActifs);
  }

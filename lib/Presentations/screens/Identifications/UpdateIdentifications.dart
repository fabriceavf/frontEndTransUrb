...
class UpdateIdentificationsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateIdentificationsScreen({Key? key ,required this.data}){
try {
...
final UpdateIdentificationsState state = Get.put(UpdateIdentificationsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Identifications',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateIdentificationsState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['statuts']?.text=data['statuts'].toString();
    ...
    await db.table('identifications').where('id','=',id).update(data);

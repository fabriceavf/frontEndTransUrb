...
class UpdateTypespointagesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypespointagesScreen({Key? key ,required this.data}){
try {
...
final UpdateTypespointagesState state = Get.put(UpdateTypespointagesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typespointages',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypespointagesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typespointages').where('id','=',id).update(data);
    var allTypespointages=await db.table('typespointages').get();
    print('allTypespointages');
    print(allTypespointages);

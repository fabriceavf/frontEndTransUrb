...
class UpdateListesjoursScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateListesjoursScreen({Key? key ,required this.data}){
try {
...
final UpdateListesjoursState state = Get.put(UpdateListesjoursState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Listesjours',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateListesjoursState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('listesjours').where('id','=',id).update(data);

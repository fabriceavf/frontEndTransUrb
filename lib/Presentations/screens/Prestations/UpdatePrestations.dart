...
class UpdatePrestationsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdatePrestationsScreen({Key? key ,required this.data}){
try {
...
final UpdatePrestationsState state = Get.put(UpdatePrestationsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Prestations',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdatePrestationsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('prestations').where('id','=',id).update(data);
    var allPrestations=await db.table('prestations').get();
    print('allPrestations');
    print(allPrestations);

...
class TestsCardScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  TestsCardScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final TestsCardState state = Get.put(TestsCardState());
    return BlocsWidget(
...
class TestsCardState extends GetxController {
  var errors = [].obs;

...
class UpdateEchelonsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateEchelonsScreen({Key? key ,required this.data}){
try {
...
final UpdateEchelonsState state = Get.put(UpdateEchelonsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Echelons',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateEchelonsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('echelons').where('id','=',id).update(data);
    var allEchelons=await db.table('echelons').get();
    print('allEchelons');
    print(allEchelons);

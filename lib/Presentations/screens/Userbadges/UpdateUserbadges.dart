...
class UpdateUserbadgesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateUserbadgesScreen({Key? key ,required this.data}){
try {
...
final UpdateUserbadgesState state = Get.put(UpdateUserbadgesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Userbadges',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateUserbadgesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('userbadges').where('id','=',id).update(data);
    var allUserbadges=await db.table('userbadges').get();
    print('allUserbadges');
    print(allUserbadges);

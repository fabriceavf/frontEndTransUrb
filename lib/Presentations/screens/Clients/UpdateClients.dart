...
class UpdateClientsScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateClientsScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateClientsState state = Get.put(UpdateClientsState());
    state.setData(data);
...
class UpdateClientsState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('clients').where('id', '=', id).update(data);
    var allClients = await db.table('clients').get();
    print('allClients');
    print(allClients);
  }

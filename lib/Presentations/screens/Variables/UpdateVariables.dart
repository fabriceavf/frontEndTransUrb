...
class UpdateVariablesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateVariablesScreen({Key? key ,required this.data}){
try {
...
final UpdateVariablesState state = Get.put(UpdateVariablesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Variables',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateVariablesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('variables').where('id','=',id).update(data);
    var allVariables=await db.table('variables').get();
    print('allVariables');
    print(allVariables);

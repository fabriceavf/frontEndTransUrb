...
class UpdateRapportsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateRapportsScreen({Key? key ,required this.data}){
try {
...
final UpdateRapportsState state = Get.put(UpdateRapportsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Rapports',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateRapportsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('rapports').where('id','=',id).update(data);

...
int? id = 0;
Map data;
UpdateServicesScreen({Key? key ,required this.data}){
try {
...
final UpdateServicesState state = Get.put(UpdateServicesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Services',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateServicesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('services').where('id','=',id).update(data);
    var allServices=await db.table('services').get();
    print('allServices');
    print(allServices);

...
class UpdateListesappelsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateListesappelsScreen({Key? key ,required this.data}){
try {
...
final UpdateListesappelsState state = Get.put(UpdateListesappelsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Listesappels',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateListesappelsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('listesappels').where('id','=',id).update(data);
    var allListesappels=await db.table('listesappels').get();
    print('allListesappels');
    print(allListesappels);

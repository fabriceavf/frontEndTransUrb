...
class UpdateInterventionimagesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateInterventionimagesScreen({Key? key ,required this.data}){
try {
...
final UpdateInterventionimagesState state = Get.put(UpdateInterventionimagesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Interventionimages',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateInterventionimagesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('interventionimages').where('id','=',id).update(data);
    var allInterventionimages=await db.table('interventionimages').get();
    print('allInterventionimages');
    print(allInterventionimages);

...
class UpdateProgrammationsusersScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateProgrammationsusersScreen({Key? key ,required this.data}){
try {
...
final UpdateProgrammationsusersState state = Get.put(UpdateProgrammationsusersState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Programmationsusers',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateProgrammationsusersState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('programmationsusers').where('id','=',id).update(data);
    var allProgrammationsusers=await db.table('programmationsusers').get();
    print('allProgrammationsusers');
    print(allProgrammationsusers);

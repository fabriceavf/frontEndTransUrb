...
class UpdateContratspostesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateContratspostesScreen({Key? key ,required this.data}){
try {
...
final UpdateContratspostesState state = Get.put(UpdateContratspostesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Contratspostes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateContratspostesState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['contratssite_id']?.text=data['contratssite_id'].toString();
    ...
    await db.table('contratspostes').where('id','=',id).update(data);

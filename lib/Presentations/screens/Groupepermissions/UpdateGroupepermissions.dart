...
class UpdateGroupepermissionsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateGroupepermissionsScreen({Key? key ,required this.data}){
try {
...
final UpdateGroupepermissionsState state = Get.put(UpdateGroupepermissionsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Groupepermissions',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateGroupepermissionsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('groupepermissions').where('id','=',id).update(data);
    var allGroupepermissions=await db.table('groupepermissions').get();
    print('allGroupepermissions');
    print(allGroupepermissions);

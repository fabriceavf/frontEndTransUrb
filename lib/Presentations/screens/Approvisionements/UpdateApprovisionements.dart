...
class UpdateApprovisionementsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateApprovisionementsScreen({Key? key ,required this.data}){
try {
...
final UpdateApprovisionementsState state = Get.put(UpdateApprovisionementsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Approvisionements',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateApprovisionementsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('approvisionements').where('id','=',id).update(data);

...
class UpdatePostesarticlesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdatePostesarticlesScreen({Key? key ,required this.data}){
try {
...
final UpdatePostesarticlesState state = Get.put(UpdatePostesarticlesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Postesarticles',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdatePostesarticlesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('postesarticles').where('id','=',id).update(data);
    var allPostesarticles=await db.table('postesarticles').get();
    print('allPostesarticles');
    print(allPostesarticles);

...
class UpdateListingsjoursScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateListingsjoursScreen({Key? key ,required this.data}){
try {
...
final UpdateListingsjoursState state = Get.put(UpdateListingsjoursState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Listingsjours',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateListingsjoursState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('listingsjours').where('id','=',id).update(data);
    var allListingsjours=await db.table('listingsjours').get();
    print('allListingsjours');
    print(allListingsjours);

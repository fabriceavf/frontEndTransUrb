...
class UpdatePastillesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdatePastillesScreen({Key? key ,required this.data}){
try {
...
final UpdatePastillesState state = Get.put(UpdatePastillesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Pastilles',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdatePastillesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('pastilles').where('id','=',id).update(data);
    var allPastilles=await db.table('pastilles').get();
    print('allPastilles');
    print(allPastilles);

...
class UpdateOnlinesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateOnlinesScreen({Key? key ,required this.data}){
try {
...
final UpdateOnlinesState state = Get.put(UpdateOnlinesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Onlines',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateOnlinesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('onlines').where('id','=',id).update(data);
    var allOnlines=await db.table('onlines').get();
    print('allOnlines');
    print(allOnlines);

...
class CreateLignesmoyenstransportsScreen extends StatelessWidget {
int? id = 0;

CreateLignesmoyenstransportsScreen({Key? key }){
    try {
...
final CreateLignesmoyenstransportsState state = Get.put(CreateLignesmoyenstransportsState());
...
body:
    ...
    child: Text('Creer un Lignesmoyenstransports',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
...
class CreateLignesmoyenstransportsState extends GetxController {
var errors= [].obs;
...
void createLine () async{
    ...
    await db.table('lignesmoyenstransports').add(data);

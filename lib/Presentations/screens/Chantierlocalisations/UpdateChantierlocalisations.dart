...
class UpdateChantierlocalisationsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateChantierlocalisationsScreen({Key? key ,required this.data}){
try {
...
final UpdateChantierlocalisationsState state = Get.put(UpdateChantierlocalisationsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Chantierlocalisations',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateChantierlocalisationsState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['latitude']?.text=data['latitude'].toString();
            this.inputControllers['longitude']?.text=data['longitude'].toString();
    ...
    await db.table('chantierlocalisations').where('id','=',id).update(data);

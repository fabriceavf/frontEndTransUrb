...
class UpdateAlldatasScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateAlldatasScreen({Key? key ,required this.data}){
try {
...
final UpdateAlldatasState state = Get.put(UpdateAlldatasState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Alldatas',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateAlldatasState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('alldatas').where('id','=',id).update(data);
    var allAlldatas=await db.table('alldatas').get();
    print('allAlldatas');
    print(allAlldatas);

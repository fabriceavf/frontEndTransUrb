...
class UpdateRolesScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateRolesScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateRolesState state = Get.put(UpdateRolesState());
    state.setData(data);
...
class UpdateRolesState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('roles').where('id', '=', id).update(data);

...
class UpdateCalendriersScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateCalendriersScreen({Key? key ,required this.data}){
try {
...
final UpdateCalendriersState state = Get.put(UpdateCalendriersState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Calendriers',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateCalendriersState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('calendriers').where('id','=',id).update(data);
    var allCalendriers=await db.table('calendriers').get();
    print('allCalendriers');
    print(allCalendriers);

...
class UpdateNationalitesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateNationalitesScreen({Key? key ,required this.data}){
try {
...
final UpdateNationalitesState state = Get.put(UpdateNationalitesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Nationalites',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateNationalitesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('nationalites').where('id','=',id).update(data);
    var allNationalites=await db.table('nationalites').get();
    print('allNationalites');
    print(allNationalites);

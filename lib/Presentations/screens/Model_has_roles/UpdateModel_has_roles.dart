...
class UpdateModel_has_rolesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateModel_has_rolesScreen({Key? key ,required this.data}){
try {
...
final UpdateModel_has_rolesState state = Get.put(UpdateModel_has_rolesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Model_has_roles',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateModel_has_rolesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('model_has_roles').where('id','=',id).update(data);
    var allModel_has_roles=await db.table('model_has_roles').get();
    print('allModel_has_roles');
    print(allModel_has_roles);

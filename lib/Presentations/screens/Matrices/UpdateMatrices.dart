...
class UpdateMatricesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateMatricesScreen({Key? key ,required this.data}){
try {
...
final UpdateMatricesState state = Get.put(UpdateMatricesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Matrices',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateMatricesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('matrices').where('id','=',id).update(data);
    var allMatrices=await db.table('matrices').get();
    print('allMatrices');
    print(allMatrices);

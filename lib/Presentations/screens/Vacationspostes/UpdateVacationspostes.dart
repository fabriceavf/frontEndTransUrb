...
class UpdateVacationspostesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateVacationspostesScreen({Key? key ,required this.data}){
try {
...
final UpdateVacationspostesState state = Get.put(UpdateVacationspostesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Vacationspostes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateVacationspostesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('vacationspostes').where('id','=',id).update(data);
    var allVacationspostes=await db.table('vacationspostes').get();
    print('allVacationspostes');
    print(allVacationspostes);

...
class UpdateCauseracinesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateCauseracinesScreen({Key? key ,required this.data}){
try {
...
final UpdateCauseracinesState state = Get.put(UpdateCauseracinesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Causeracines',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateCauseracinesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('causeracines').where('id','=',id).update(data);
    var allCauseracines=await db.table('causeracines').get();
    print('allCauseracines');
    print(allCauseracines);

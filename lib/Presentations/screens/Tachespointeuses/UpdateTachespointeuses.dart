...
class UpdateTachespointeusesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTachespointeusesScreen({Key? key ,required this.data}){
try {
...
final UpdateTachespointeusesState state = Get.put(UpdateTachespointeusesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Tachespointeuses',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTachespointeusesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('tachespointeuses').where('id','=',id).update(data);
    var allTachespointeuses=await db.table('tachespointeuses').get();
    print('allTachespointeuses');
    print(allTachespointeuses);

...
class UpdateActionsScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateActionsScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateActionsState state = Get.put(UpdateActionsState());
    state.setData(data);
...
class UpdateActionsState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('actions').where('id', '=', id).update(data);
    var allActions = await db.table('actions').get();
    print('allActions');
    print(allActions);
  }

...
import './LogsCard.dart';
import './CreateLogs.dart';
import './UpdateLogs.dart';
...
class LogsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final LogsState c = Get.put(LogsState());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            AggridScreen(
              ...
              fields: [
                {
                  ...
                  'render': (data) => DataCell(ButtonWidget(
                        ...
                        text: 'Edit',
                        onTapCallBack: () {
                          Services.ShowModal(UpdateLogsScreen(data: data),
                ...
                },
                {
                  'libelle': 'navigateur',
                  'render': (data) =>
                      DataCell(Text(data['navigateur'].toString())),
                },
                {
                  'libelle': 'pays',
              ...
              newElementInitCallback: () {
                Services.ShowModal(CreateLogsScreen(),
                    barrierDismissible: false);
              },
              table: 'logs',
              showWidgetCallback: (data) => DataRow(
                cells: <DataCell>[
                  ...
                  DataCell(Text(data['ip'].toString())),
                  DataCell(Text(data['details'].toString())),
                  DataCell(Text(data['navigateur'].toString())),
                  DataCell(Text(data['pays'].toString())),
                  DataCell(Text(data['ville'].toString())),
                  DataCell(Text(data['user_id'].toString())),
                  DataCell(Text(data['created_at'].toString())),
                  DataCell(Text(data['updated_at'].toString())),
                  DataCell(Text(data['deleted_at'].toString())),
...
class LogsState extends GetxController {
  ...
  var formKey = 0.obs;
  var tableKey = 0.obs;
  var url = 'http://127.0.0.1:8000/api/logs-Aggrid'.obs;
  var table = 'logs'.obs;
  var usersData = [].obs;
  var requette = 1.obs;

...
class UpdateProvincesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateProvincesScreen({Key? key ,required this.data}){
try {
...
final UpdateProvincesState state = Get.put(UpdateProvincesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Provinces',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateProvincesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('provinces').where('id','=',id).update(data);
    var allProvinces=await db.table('provinces').get();
    print('allProvinces');
    print(allProvinces);

...
class UpdateExtrasdatasScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateExtrasdatasScreen({Key? key ,required this.data}){
try {
...
final UpdateExtrasdatasState state = Get.put(UpdateExtrasdatasState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Extrasdatas',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateExtrasdatasState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('extrasdatas').where('id','=',id).update(data);
    var allExtrasdatas=await db.table('extrasdatas').get();
    print('allExtrasdatas');
    print(allExtrasdatas);

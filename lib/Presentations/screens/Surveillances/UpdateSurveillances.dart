...
class UpdateSurveillancesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateSurveillancesScreen({Key? key ,required this.data}){
try {
...
final UpdateSurveillancesState state = Get.put(UpdateSurveillancesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Surveillances',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateSurveillancesState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['id_base']?.text=data['id_base'].toString();
    ...
    await db.table('surveillances').where('id','=',id).update(data);

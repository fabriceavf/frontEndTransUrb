...
class UpdateJoursferiesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateJoursferiesScreen({Key? key ,required this.data}){
try {
...
final UpdateJoursferiesState state = Get.put(UpdateJoursferiesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Joursferies',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateJoursferiesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('joursferies').where('id','=',id).update(data);

...
class UpdateTypesagentshorairesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypesagentshorairesScreen({Key? key ,required this.data}){
try {
...
final UpdateTypesagentshorairesState state = Get.put(UpdateTypesagentshorairesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typesagentshoraires',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypesagentshorairesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typesagentshoraires').where('id','=',id).update(data);
    var allTypesagentshoraires=await db.table('typesagentshoraires').get();
    print('allTypesagentshoraires');
    print(allTypesagentshoraires);

...
class DetailsScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final DetailsState c = Get.put(DetailsState());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            AggridScreen(
              ...
              fields: [
                {
                  ...
                  'render': (data) => DataCell(ButtonWidget(
                        ...
                        text: 'Edit',
                        onTapCallBack: () {
                          Services.ShowModal(UpdateDetailsScreen(data: data),
                ...
                {
                  'libelle': 'order',
                  'render': (data) => DataCell(Text(data['order'].toString())),
                },
                {
                  'libelle': 'processu_id',
                  'render': (data) =>
                      DataCell(Text(data['processu_id'].toString())),
                },
                {
                  'libelle': 'created_at',
                  'render': (data) =>
              ...
              newElementInitCallback: () {
                Services.ShowModal(CreateDetailsScreen(),
                    barrierDismissible: false);
              },
              table: 'details',
...
class DetailsState extends GetxController {
  var isLoading = false.obs;
  var gridApi = null.obs;
  var count = 0.obs;
  var formId = "details".obs;
  var formData = {}.obs;
  var formState = "".obs;
  var formWidth = 'lg'.obs;
  var formKey = 0.obs;
  var tableKey = 0.obs;
  var url = 'http://127.0.0.1:8000/api/details-Aggrid'.obs;
  var table = 'details'.obs;
  var processusData = [].obs;
  var requette = 1.obs;

...
class UpdateDetailsScreen extends StatelessWidget {
  ...
  UpdateDetailsScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateDetailsState state = Get.put(UpdateDetailsState());
    state.setData(data);
              ...
              child: Column(
                ...
                children: <Widget>[
                  ...
                  Obx(() => MyInputWidget(
                      ...
                      valid: 0)),
                  Obx(() => MyInputWidget(
                      controller: state.inputControllers['order'],
                      label: "order",
                      placeHolder: "",
                      valid: 0)),
                  Obx(() => MyInputWidget(
                  ...
                  FieldSelectWidget(
                    label: "processus",
                    detail: "Veuillez selectionner processus",
                    placeHolder: "",
                    baseData: [],
                    model: state.formProcessu_id,
                    url: 'processus-Aggrid',
                    filterFields: [],
...
class UpdateDetailsState extends GetxController {
  var errors = [].obs;
  ...
  var inputControllers = {
    ...
    'processu_id': TextEditingController(),
  ...
  void setData(data) {
    ...
    this.inputControllers['order']?.text = data['order'].toString();
    this.inputControllers['processu_id']?.text = data['processu_id'].toString();
  ...
  void UpdateLine() async {
    ...
    await db.table('details').where('id', '=', id).update(data);
    var allDetails = await db.table('details').get();
    print('allDetails');
    print(allDetails);
  }
  ...
  void resetForm() {
    ...
    this.inputControllers['order']?.text = "";
    this.inputControllers['processu_id']?.text = "";
  ...
  Map<String, dynamic> getForm() {
    return {
      ...
      'order': this.inputControllers['order']?.text,
      'processu_id': this.inputControllers['processu_id']?.text,

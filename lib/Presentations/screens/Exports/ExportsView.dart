...
import './ExportsCard.dart';
import './CreateExports.dart';
import './UpdateExports.dart';

class ExportsScreen extends StatelessWidget {
  int? id = 0;

  ExportsScreen({Key? key}) {
    try {
  ...
  Widget build(BuildContext context) {
    final ExportsState c = Get.put(ExportsState());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            AggridScreen(
              ...
              fields: [
                {
                  ...
                  'render': (data) => DataCell(ButtonWidget(
                        ...
                        text: 'Edit',
                        onTapCallBack: () {
                          Services.ShowModal(UpdateExportsScreen(data: data),
                ...
                {
                  'libelle': 'lien',
                  'render': (data) => DataCell(Text(data['lien'].toString())),
                },
                {
                  'libelle': 'creat_by',
                  'render': (data) =>
              ...
              newElementInitCallback: () {
                Services.ShowModal(CreateExportsScreen(),
                    barrierDismissible: false);
              },
              table: 'exports',
              showWidgetCallback: (data) => DataRow(
                cells: <DataCell>[
                  ...
                  DataCell(Text(data['libelle'].toString())),
                  DataCell(Text(data['lien'].toString())),
                  DataCell(Text(data['creat_by'].toString())),
                  DataCell(Text(data['extra_attributes'].toString())),
                  DataCell(Text(data['created_at'].toString())),
                  DataCell(Text(data['updated_at'].toString())),
...
class ExportsState extends GetxController {
  var isLoading = false.obs;
  var gridApi = null.obs;
  var count = 0.obs;
  var formId = "exports".obs;
  var formData = {}.obs;
  var formState = "".obs;
  var formWidth = 'lg'.obs;
  var formKey = 0.obs;
  var tableKey = 0.obs;
  var url = 'http://127.0.0.1:8000/api/exports-Aggrid'.obs;
  var table = 'exports'.obs;
  var requette = 0.obs;

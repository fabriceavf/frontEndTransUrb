...
class UpdateSituationsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateSituationsScreen({Key? key ,required this.data}){
try {
...
final UpdateSituationsState state = Get.put(UpdateSituationsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Situations',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateSituationsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('situations').where('id','=',id).update(data);
    var allSituations=await db.table('situations').get();
    print('allSituations');
    print(allSituations);

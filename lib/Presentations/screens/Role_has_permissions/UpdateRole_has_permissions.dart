...
class UpdateRole_has_permissionsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateRole_has_permissionsScreen({Key? key ,required this.data}){
try {
...
final UpdateRole_has_permissionsState state = Get.put(UpdateRole_has_permissionsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Role_has_permissions',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateRole_has_permissionsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('role_has_permissions').where('id','=',id).update(data);

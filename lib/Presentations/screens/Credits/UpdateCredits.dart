...
class UpdateCreditsScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateCreditsScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateCreditsState state = Get.put(UpdateCreditsState());
    state.setData(data);
...
class UpdateCreditsState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('credits').where('id', '=', id).update(data);
    var allCredits = await db.table('credits').get();
    print('allCredits');
    print(allCredits);
  }

...
class UpdateTypessitesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypessitesScreen({Key? key ,required this.data}){
try {
...
final UpdateTypessitesState state = Get.put(UpdateTypessitesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typessites',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypessitesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typessites').where('id','=',id).update(data);
    var allTypessites=await db.table('typessites').get();
    print('allTypessites');
    print(allTypessites);

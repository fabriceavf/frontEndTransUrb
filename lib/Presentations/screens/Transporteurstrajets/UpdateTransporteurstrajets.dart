...
class UpdateTransporteurstrajetsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTransporteurstrajetsScreen({Key? key ,required this.data}){
try {
...
final UpdateTransporteurstrajetsState state = Get.put(UpdateTransporteurstrajetsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Transporteurstrajets',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTransporteurstrajetsState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['transporteur_id']?.text=data['transporteur_id'].toString();
            this.inputControllers['trajet_id']?.text=data['trajet_id'].toString();
    ...
    await db.table('transporteurstrajets').where('id','=',id).update(data);

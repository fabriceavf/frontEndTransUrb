...
class UpdateTransactionspostessynthesesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTransactionspostessynthesesScreen({Key? key ,required this.data}){
try {
...
final UpdateTransactionspostessynthesesState state = Get.put(UpdateTransactionspostessynthesesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Transactionspostessyntheses',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTransactionspostessynthesesState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['transactions_matricule']?.text=data['transactions_matricule'].toString();
    ...
    await db.table('transactionspostessyntheses').where('id','=',id).update(data);

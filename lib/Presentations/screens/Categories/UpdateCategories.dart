...
int? id = 0;
Map data;
UpdateCategoriesScreen({Key? key ,required this.data}){
try {
...
final UpdateCategoriesState state = Get.put(UpdateCategoriesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Categories',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateCategoriesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('categories').where('id','=',id).update(data);
    var allCategories=await db.table('categories').get();
    print('allCategories');
    print(allCategories);

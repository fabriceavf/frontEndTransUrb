...
class UpdateMatrimonialesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateMatrimonialesScreen({Key? key ,required this.data}){
try {
...
final UpdateMatrimonialesState state = Get.put(UpdateMatrimonialesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Matrimoniales',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateMatrimonialesState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['remember_token']?.text=data['remember_token'].toString();
    ...
    await db.table('matrimoniales').where('id','=',id).update(data);
    var allMatrimoniales=await db.table('matrimoniales').get();
    print('allMatrimoniales');
    print(allMatrimoniales);

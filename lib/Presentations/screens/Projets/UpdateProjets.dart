...
class UpdateProjetsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateProjetsScreen({Key? key ,required this.data}){
try {
...
final UpdateProjetsState state = Get.put(UpdateProjetsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Projets',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateProjetsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('projets').where('id','=',id).update(data);

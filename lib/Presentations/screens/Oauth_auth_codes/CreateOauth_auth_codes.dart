...
class CreateOauth_auth_codesScreen extends StatelessWidget {
int? id = 0;

CreateOauth_auth_codesScreen({Key? key }){
    try {
...
final CreateOauth_auth_codesState state = Get.put(CreateOauth_auth_codesState());
...
body:
    ...
    child: Text('Creer un Oauth_auth_codes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
...
class CreateOauth_auth_codesState extends GetxController {
var errors= [].obs;
...
void createLine () async{
    ...
    await db.table('oauth_auth_codes').add(data);

...
class UpdateVilleszonesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateVilleszonesScreen({Key? key ,required this.data}){
try {
...
final UpdateVilleszonesState state = Get.put(UpdateVilleszonesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Villeszones',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateVilleszonesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('villeszones').where('id','=',id).update(data);

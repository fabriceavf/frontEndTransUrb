...
class UpdateGroupedirectionsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateGroupedirectionsScreen({Key? key ,required this.data}){
try {
...
final UpdateGroupedirectionsState state = Get.put(UpdateGroupedirectionsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Groupedirections',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateGroupedirectionsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('groupedirections').where('id','=',id).update(data);

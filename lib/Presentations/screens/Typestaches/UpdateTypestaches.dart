...
class UpdateTypestachesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypestachesScreen({Key? key ,required this.data}){
try {
...
final UpdateTypestachesState state = Get.put(UpdateTypestachesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typestaches',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypestachesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typestaches').where('id','=',id).update(data);
    var allTypestaches=await db.table('typestaches').get();
    print('allTypestaches');
    print(allTypestaches);

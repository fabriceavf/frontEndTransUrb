...
class UpdatePointeusestransactionsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdatePointeusestransactionsScreen({Key? key ,required this.data}){
try {
...
final UpdatePointeusestransactionsState state = Get.put(UpdatePointeusestransactionsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Pointeusestransactions',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdatePointeusestransactionsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('pointeusestransactions').where('id','=',id).update(data);

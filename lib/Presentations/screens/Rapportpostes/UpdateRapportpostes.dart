...
class UpdateRapportpostesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateRapportpostesScreen({Key? key ,required this.data}){
try {
...
final UpdateRapportpostesState state = Get.put(UpdateRapportpostesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Rapportpostes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateRapportpostesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('rapportpostes').where('id','=',id).update(data);

...
class UpdateTypesventilationsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypesventilationsScreen({Key? key ,required this.data}){
try {
...
final UpdateTypesventilationsState state = Get.put(UpdateTypesventilationsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typesventilations',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypesventilationsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typesventilations').where('id','=',id).update(data);
    var allTypesventilations=await db.table('typesventilations').get();
    print('allTypesventilations');
    print(allTypesventilations);

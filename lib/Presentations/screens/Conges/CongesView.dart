...
import './CongesCard.dart';
import './CreateConges.dart';
import './UpdateConges.dart';

class CongesScreen extends StatelessWidget {
  int? id = 0;

  CongesScreen({Key? key}) {
    try {
  ...
  Widget build(BuildContext context) {
    final CongesState c = Get.put(CongesState());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            AggridScreen(
              ...
              fields: [
                {
                  ...
                  'render': (data) => DataCell(ButtonWidget(
                        ...
                        text: 'Edit',
                        onTapCallBack: () {
                          Services.ShowModal(UpdateCongesScreen(data: data),
              ...
              newElementInitCallback: () {
                Services.ShowModal(CreateCongesScreen(),
                    barrierDismissible: false);
              },
              table: 'conges',
              showWidgetCallback: (data) => DataRow(
                cells: <DataCell>[
                  ...
                  DataCell(Text(data['user_id'].toString())),
                  DataCell(Text(data['raison'].toString())),
...
class CongesState extends GetxController {
  var isLoading = false.obs;
  var gridApi = null.obs;
  var count = 0.obs;
  var formId = "conges".obs;
  var formData = {}.obs;
  var formState = "".obs;
  var formWidth = 'lg'.obs;
  var formKey = 0.obs;
  var tableKey = 0.obs;
  var url = 'http://127.0.0.1:8000/api/conges-Aggrid'.obs;
  var table = 'conges'.obs;
  var usersData = [].obs;
  var requette = 1.obs;

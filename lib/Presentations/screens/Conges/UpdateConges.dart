...
class UpdateCongesScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateCongesScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateCongesState state = Get.put(UpdateCongesState());
    state.setData(data);
...
class UpdateCongesState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('conges').where('id', '=', id).update(data);

...
class UpdatePostespointeusesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdatePostespointeusesScreen({Key? key ,required this.data}){
try {
...
final UpdatePostespointeusesState state = Get.put(UpdatePostespointeusesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Postespointeuses',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdatePostespointeusesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('postespointeuses').where('id','=',id).update(data);

...
class UpdateHistoriquesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateHistoriquesScreen({Key? key ,required this.data}){
try {
...
final UpdateHistoriquesState state = Get.put(UpdateHistoriquesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Historiques',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateHistoriquesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('historiques').where('id','=',id).update(data);

...
class UpdateRecuperesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateRecuperesScreen({Key? key ,required this.data}){
try {
...
final UpdateRecuperesState state = Get.put(UpdateRecuperesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Recuperes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateRecuperesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('recuperes').where('id','=',id).update(data);
    var allRecuperes=await db.table('recuperes').get();
    print('allRecuperes');
    print(allRecuperes);

...
class UpdateActionsprevisionellesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateActionsprevisionellesScreen({Key? key ,required this.data}){
try {
...
final UpdateActionsprevisionellesState state = Get.put(UpdateActionsprevisionellesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Actionsprevisionelles',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateActionsprevisionellesState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['besoin_id']?.text=data['besoin_id'].toString();
    ...
    await db.table('actionsprevisionelles').where('id','=',id).update(data);

...
class CreateSupervirzclientsScreen extends StatelessWidget {
int? id = 0;

CreateSupervirzclientsScreen({Key? key }){
    try {
...
final CreateSupervirzclientsState state = Get.put(CreateSupervirzclientsState());
...
body:
    ...
    child: Text('Creer un Supervirzclients',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
...
class CreateSupervirzclientsState extends GetxController {
var errors= [].obs;
...
void createLine () async{
    ...
    await db.table('supervirzclients').add(data);

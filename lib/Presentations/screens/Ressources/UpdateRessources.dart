...
class UpdateRessourcesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateRessourcesScreen({Key? key ,required this.data}){
try {
...
final UpdateRessourcesState state = Get.put(UpdateRessourcesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Ressources',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateRessourcesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('ressources').where('id','=',id).update(data);
    var allRessources=await db.table('ressources').get();
    print('allRessources');
    print(allRessources);

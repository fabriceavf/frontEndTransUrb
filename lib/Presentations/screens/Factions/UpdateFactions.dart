...
class UpdateFactionsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateFactionsScreen({Key? key ,required this.data}){
try {
...
final UpdateFactionsState state = Get.put(UpdateFactionsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Factions',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateFactionsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('factions').where('id','=',id).update(data);
    var allFactions=await db.table('factions').get();
    print('allFactions');
    print(allFactions);

...
class UpdateTraitementsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTraitementsScreen({Key? key ,required this.data}){
try {
...
final UpdateTraitementsState state = Get.put(UpdateTraitementsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Traitements',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTraitementsState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['etat_depart']?.text=data['etat_depart'].toString();
            this.inputControllers['etat_arrive']?.text=data['etat_arrive'].toString();
    ...
    await db.table('traitements').where('id','=',id).update(data);

...
class UpdateFacturationuploadsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateFacturationuploadsScreen({Key? key ,required this.data}){
try {
...
final UpdateFacturationuploadsState state = Get.put(UpdateFacturationuploadsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Facturationuploads',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateFacturationuploadsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('facturationuploads').where('id','=',id).update(data);
    var allFacturationuploads=await db.table('facturationuploads').get();
    print('allFacturationuploads');
    print(allFacturationuploads);

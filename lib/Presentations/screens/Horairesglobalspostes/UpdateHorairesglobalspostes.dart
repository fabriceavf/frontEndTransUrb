...
class UpdateHorairesglobalspostesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateHorairesglobalspostesScreen({Key? key ,required this.data}){
try {
...
final UpdateHorairesglobalspostesState state = Get.put(UpdateHorairesglobalspostesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Horairesglobalspostes',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateHorairesglobalspostesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('horairesglobalspostes').where('id','=',id).update(data);

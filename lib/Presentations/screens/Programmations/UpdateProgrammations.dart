...
class UpdateProgrammationsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateProgrammationsScreen({Key? key ,required this.data}){
try {
...
final UpdateProgrammationsState state = Get.put(UpdateProgrammationsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Programmations',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateProgrammationsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('programmations').where('id','=',id).update(data);

...
class UpdateMesurespreventivesScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateMesurespreventivesScreen({Key? key ,required this.data}){
try {
...
final UpdateMesurespreventivesState state = Get.put(UpdateMesurespreventivesState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Mesurespreventives',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateMesurespreventivesState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('mesurespreventives').where('id','=',id).update(data);

...
class UpdateMaterielprevisionnelsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateMaterielprevisionnelsScreen({Key? key ,required this.data}){
try {
...
final UpdateMaterielprevisionnelsState state = Get.put(UpdateMaterielprevisionnelsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Materielprevisionnels',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateMaterielprevisionnelsState extends GetxController {
    var errors= [].obs;
    ...
    void setData(data){
            ...
            this.inputControllers['chantier_id']?.text=data['chantier_id'].toString();
    ...
    await db.table('materielprevisionnels').where('id','=',id).update(data);
    var allMaterielprevisionnels=await db.table('materielprevisionnels').get();
    print('allMaterielprevisionnels');
    print(allMaterielprevisionnels);

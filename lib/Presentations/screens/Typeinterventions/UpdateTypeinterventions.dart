...
class UpdateTypeinterventionsScreen extends StatelessWidget {
int? id = 0;
Map data;
UpdateTypeinterventionsScreen({Key? key ,required this.data}){
try {
...
final UpdateTypeinterventionsState state = Get.put(UpdateTypeinterventionsState());
state.setData(data);
...
children: <Widget>[
    ...
    child: Text('Update un Typeinterventions',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
    ...
    class UpdateTypeinterventionsState extends GetxController {
    var errors= [].obs;
    ...
    await db.table('typeinterventions').where('id','=',id).update(data);
    var allTypeinterventions=await db.table('typeinterventions').get();
    print('allTypeinterventions');
    print(allTypeinterventions);

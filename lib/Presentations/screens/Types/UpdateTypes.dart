...
class UpdateTypesScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateTypesScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateTypesState state = Get.put(UpdateTypesState());
    state.setData(data);
...
class UpdateTypesState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('types').where('id', '=', id).update(data);
    var allTypes = await db.table('types').get();
    print('allTypes');
    print(allTypes);
  }

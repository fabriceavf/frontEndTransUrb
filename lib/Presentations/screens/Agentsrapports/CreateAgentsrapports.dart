...
class CreateAgentsrapportsScreen extends StatelessWidget {
int? id = 0;

CreateAgentsrapportsScreen({Key? key }){
    try {
...
final CreateAgentsrapportsState state = Get.put(CreateAgentsrapportsState());
...
body:
    ...
    child: Text('Creer un Agentsrapports',style: TextStyle(fontSize: Constants.size['large']['fontSize'].toDouble()),),
    ),
...
class CreateAgentsrapportsState extends GetxController {
var errors= [].obs;
...
void createLine () async{
    ...
    await db.table('agentsrapports').add(data);

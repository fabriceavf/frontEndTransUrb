...
import './WorksCard.dart';
import './CreateWorks.dart';
import './UpdateWorks.dart';
...
class WorksScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final WorksState c = Get.put(WorksState());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            AggridScreen(
              ...
              fields: [
                {
                  ...
                  'render': (data) => DataCell(ButtonWidget(
                        ...
                        text: 'Edit',
                        onTapCallBack: () {
                          Services.ShowModal(UpdateWorksScreen(data: data),
                ...
                },
                {
                  'libelle': 'activite_id',
                  'render': (data) =>
                      DataCell(Text(data['activite_id'].toString())),
                },
                {
                  'libelle': 'user_id',
                  'render': (data) =>
                ...
                {
                  'libelle': 'groupe',
                  'render': (data) => DataCell(Text(data['groupe'].toString())),
                },
                {
              ...
              newElementInitCallback: () {
                Services.ShowModal(CreateWorksScreen(),
                    barrierDismissible: false);
              },
              table: 'works',
              showWidgetCallback: (data) => DataRow(
                cells: <DataCell>[
                  ...
                  DataCell(Text(data['fin'].toString())),
                  DataCell(Text(data['groupe'].toString())),
                  DataCell(Text(data['identifiants_sadge'].toString())),
                  DataCell(Text(data['creat_by'].toString())),
                ],
              ),
...
class WorksState extends GetxController {
  var isLoading = false.obs;
  var gridApi = null.obs;
  var count = 0.obs;
  var formId = "works".obs;
  var formData = {}.obs;
  var formState = "".obs;
  var formWidth = 'lg'.obs;
  var formKey = 0.obs;
  var tableKey = 0.obs;
  var url = 'http://127.0.0.1:8000/api/works-Aggrid'.obs;
  var table = 'works'.obs;

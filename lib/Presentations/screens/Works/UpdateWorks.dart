...
class UpdateWorksScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateWorksScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateWorksState state = Get.put(UpdateWorksState());
    state.setData(data);
...
class UpdateWorksState extends GetxController {
  var errors = [].obs;
  ...
  void setData(data) {
    ...
    this.inputControllers['groupe']?.text = data['groupe'].toString();
  ...
  void UpdateLine() async {
    ...
    await db.table('works').where('id', '=', id).update(data);

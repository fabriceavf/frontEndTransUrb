...
import './EtapesCard.dart';
import './CreateEtapes.dart';
import './UpdateEtapes.dart';

class EtapesScreen extends StatelessWidget {
  int? id = 0;

  EtapesScreen({Key? key}) {
    try {
  ...
  Widget build(BuildContext context) {
    final EtapesState c = Get.put(EtapesState());
    return Scaffold(
      appBar: AppBar(
          ...
          children: [
            AggridScreen(
              ...
              fields: [
                {
                  ...
                  'render': (data) => DataCell(ButtonWidget(
                        ...
                        text: 'Edit',
                        onTapCallBack: () {
                          Services.ShowModal(UpdateEtapesScreen(data: data),
                ...
                },
                {
                  'libelle': 'ligne_id',
                  'render': (data) =>
                      DataCell(Text(data['ligne_id'].toString())),
                },
                {
                  'libelle': 'creat_by',
                  'render': (data) =>
              ...
              newElementInitCallback: () {
                Services.ShowModal(CreateEtapesScreen(),
                    barrierDismissible: false);
              },
              table: 'etapes',
...
class EtapesState extends GetxController {
  var isLoading = false.obs;
  var gridApi = null.obs;
  var count = 0.obs;
  var formId = "etapes".obs;
  var formData = {}.obs;
  var formState = "".obs;
  var formWidth = 'lg'.obs;
  var formKey = 0.obs;
  var tableKey = 0.obs;
  var url = 'http://127.0.0.1:8000/api/etapes-Aggrid'.obs;
  var table = 'etapes'.obs;
  var lignesData = [].obs;
  var requette = 1.obs;

...
class UpdateVillesScreen extends StatelessWidget {
  int? id = 0;
  Map data;

  UpdateVillesScreen({Key? key, required this.data}) {
    try {
  ...
  Widget build(BuildContext context) {
    final UpdateVillesState state = Get.put(UpdateVillesState());
    state.setData(data);
...
class UpdateVillesState extends GetxController {
  var errors = [].obs;
  ...
  void UpdateLine() async {
    ...
    await db.table('villes').where('id', '=', id).update(data);
    var allVilles = await db.table('villes').get();
    print('allVilles');
    print(allVilles);
  }

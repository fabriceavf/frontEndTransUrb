...
class SqlLiteDB implements DatabaseStructure {
  ...
  Future<String> checkPermissionsAndParseJsonFile() async {
    try {
      ...
      if (externalDir == null) {
        return 'Could not access external directory';
      }
      String externalPath = externalDir.path;
      File file = File('$externalPath/Documents/sqls.json');
      ...
      if (await file.exists()) {
        ...
        return 'sqls.json file not found';
      }
    } catch (e) {
      return 'Error: $e';
    }
  ...
  static Future<Database?> init() async {
    ...
    String path = databasesPath + '/demoTest.db';
    print('voila le path');
// open the database
    dbInstance = await openDatabase(path, version: 2,
...
// When creating the db, create the table
    ...
    await SqlLiteDB.loadData();

  }

  static dynamic loadData() async {
    try {
      var status = await Permission.storage.status;
      if (!status.isGranted) {
        status = await Permission.storage.request();
      ...
      if (await file.exists()) {
        ...
        for (var table in jsonContent.keys) {
          ...
          for (var command in sqlCommands) {
            i=i+1;
            print('voici la commande =>'+i.toString()+'/'+sqlCommands.length.toString());
            try{
              await dbInstance?.transaction((txn) async {
                await txn.rawQuery(command);
              });
            } catch (e) {
            print('Error $e');
            }
  ...
  delete() async {
    var where = [];
    var set = [];
    whereDatas.forEach((element) {
      print('coic lelement');
      print(element);
      where.add(element['champ'].toString() +
          ' ' +
...
// Insert some records in a transaction
  ...
  @override
  SqlLiteDB table(String table) {
    print('set Table de isarDb');
    this.tableName = table;
    return this;
  }

  SqlLiteDB limit(int limit) {
    print('set limit de isarDb');
    this.limitRow = limit;
    return this;
  }

  SqlLiteDB offset(int offset) {
    print('set offset de isarDb');
    this.offsetRow = offset;
    return this;
  }

  SqlLiteDB where(String champ, String type, String value) {
    this.whereDatas.add({
      'champ': champ,
      'value': value,
      'type': type,
  ...
  SqlLiteDB orderBy(String champ, String type) {
    this.orderByDatas.add({
      'champ': champ,
      'type': type,
    });
    return this;
...
// Insert some records in a transaction
  ...
  get() async {
    ...
    await dbInstance?.transaction((txn) async {
      ...
      orderByDatas.forEach((element) {
        order.add([element['champ'].toString(), element['type'].toString()]
            .join(" "));
      });
      List getParams = [
        'SELECT * FROM ',
      ...
      if (orderByDatas.length > 0) {
        getParams.add('ORDER BY');
        getParams.add(order.join(' , '));
      }
      // getParams.add(' limit ');
      // getParams.add(limitRow);
      // getParams.add(' offset ');
      // getParams.add(offsetRow);
      getParams.add(' ; ');

      String sql = getParams.join(' ');
      print(sql);
...
// TODO: implement getAll
  ...
  sync() async {
   SqlLiteDB.loadData();
    DatabaseDto $dbDtoTransactions=DatabaseManager.getDto();
    $dbDtoTransactions=DatabaseManager.setTable($dbDtoTransactions, 'transactions');
    $dbDtoTransactions=await DatabaseManager.read($dbDtoTransactions, ['*']);
    List transactions = $dbDtoTransactions.result;
    print('voici les transactions');
    print(transactions.length);
    if(transactions.length>=1){
    // if(true){
      Map<String,List> champs={
        'abscences':['id','user_id','raison','debut','fin','etats','typesabscence_id','extra_attributes','created_at','updated_at','valide','deleted_at','identifiants_sadge','creat_by',],
        'actifs':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'actions':['id','libelle','description','work_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'actionsprevisionelles':['id','libelle','descriptions','debut_previsionnel','fin_previsionnel','debut_reel','fin_reel','besoin_id','creat_by','evaluation','valider','type','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge',],
        'actionsrealises':['id','libelle','descriptions','debut_previsionnel','fin_previsionnel','debut_reel','fin_reel','actionsprevisionelle_id','creat_by','evaluation','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge',],
        'activites':['id','libelle','created_at','updated_at','extra_attributes','deleted_at','duree','parent','user_id','has_child','description','validate','type','etats_actuel','description_actuel','ParentElements','AllEtats','CanUpdate','IsCreateByMe','IsWorkForMe','Status','Createur','identifiants_sadge','creat_by',],
        'agentsrapports':['id','mois','user_id','jour_abscences','jour_presences','day_01','day_02','day_03','day_04','day_05','day_06','day_07','day_08','day_09','day_10','day_11','day_12','day_13','day_14','day_15','day_16','day_17','day_18','day_19','day_20','day_21','day_22','day_23','day_24','day_25','day_26','day_27','day_28','day_29','day_30','day_31','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'alarms':['id','libelle','description','type','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'alldatas':['id','cle','valeur','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'analysespointeuses':['id','pointeuses','semaine','lun','mar','mer','jeu','ven','sam','dim','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'approvisionementdetails':['id','approvisionement_id','quantite','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'approvisionements':['id','libelle','date','created_at','updated_at','valider','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'assignations':['id','date','user_id','carte_id','debut','fin','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'attributions':['id','ressource_id','user_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'badges':['id','client_id','content','created_at','updated_at','js','libelle','css','node_version','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'balises':['id','imei','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by','libelle','ref',],
        'besoins':['id','libelle','descriptions','debut_previsionnel','fin_previsionnel','debut_reel','fin_reel','projet_id','evaluation','creat_by','valider','created_at','updated_at','extra_attributes','deleted_at','Child','ChildPrevu','ChildImprevu','ChildReussi','ChildBloquer','identifiants_sadge',],
        'calendriers':['id','libelle','type','description','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'cartes':['id','code','uid_mifare','solde','site_id','etats','deleted_at','creat_by','identifiants_sadge','extra_attributes','created_at','updated_at',],
        'categories':['id','libelle','code','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'causeracines':['id','libelle','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'chantierlocalisations':['id','chantier_id','latitude','longitude','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'chantiers':['id','libelle','couleur','debut_prevus','fin_prevus','debut_effectif','fin_effectif','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'clients':['id','code','libelle','created_at','updated_at','type','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'configurations':['id','cle','valeur','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'conges':['id','user_id','raison','debut','fin','etats','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'contrats':['id','libelle','code','extra_attributes','created_at','updated_at','typeView','expiration','deleted_at','identifiants_sadge','creat_by',],
        'contratsagents':['id','contratsposte_id','user_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'contratsclients':['id','libelle','description','client_id','created_at','updated_at','extra_attributes','deleted_at','AllSites','identifiants_sadge','creat_by',],
        'contratspostes':['id','contratssite_id','poste_id','jours','agentsjour','agentsnuit','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'contratssites':['id','contratsclient_id','site_id','prestation_id','agentsjour','agentsnuit','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'controlleursacces':['id','pointeuse_id','ligne_id','deplacement_id','site_id','date_debut','date_fin','creat_by','extra_attributes','created_at','updated_at','deleted_at','type',],
        'credits':['id','identification_id','montant','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'cruds':['id','action','entite','entite_cle','ancien','nouveau','user_id','created_at','updated_at','deleted_at','extra_attributes','Detail','identifiants_sadge','creat_by',],
        'debits':['id','identification_id','montant','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'dependances':['id','badge_id','libelle','created_at','updated_at','version','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'deplacements':['id','date','debut_prevu','fin_prevu','lignesmoyenstransport_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','moyenstransport_id','ligne_id',],
        'deploiementspointeusesmoyenstransports':['id','date','pointeuse_id','moyenstransport_id','debut','fin','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'details':['id','libelle','description','order','processu_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'directions':['id','libelle','code','extra_attributes','created_at','updated_at','groupedirection_id','deleted_at','identifiants_sadge','creat_by',],
        'documents':['id','nom','rubrique','fichier','agent_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'doublonsPostes':['id','ancienHoraire','nouveauHoraire','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'echelons':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'ecouteurs':['id','avant','apres','attribut','created_at','updated_at','agent_id','user_id','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'empreintes':['id','signature','user_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'entreprises':['id','nom','menu','host','extra_attributes','created_at','updated_at','deleted_at','icon','favicon','status','identifiants_sadge','creat_by','db_host','db_user','db_pass','badge_avant','badge_arriere','modules','filemodules',],
        'etapes':['id','libelle','ordre','ligne_id','creat_by','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge',],
        'exports':['id','code','libelle','lien','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'exportsdetails':['id','export_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'extrasdatas':['id','cle','valeur','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'factions':['id','libelle','code','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'facturationuploads':['id','libelle','path','extra_attributes','deleted_at','created_at','updated_at','identifiants_sadge','creat_by',],
        'files':['id','old_name','new_name','descriptions','extensions','size','path','web_path','statut','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'fonctions':['id','code','libelle','extra_attributes','created_at','updated_at','service_id','deleted_at','identifiants_sadge','creat_by',],
        'forms':['id','libelle','description','childs','champs','extra_attributes','creat_by','deleted_at','created_at','updated_at','identifiants_sadge',],
        'formschamps':['id','libelle','description','type','cle','width','extra_attributes','creat_by','deleted_at','created_at','updated_at','identifiants_sadge',],
        'formsdatas':['id','libelle','parent','form_id','cle0','cle1','cle2','cle3','cle4','cle5','cle6','cle7','cle8','cle9','cle10','cle11','cle12','cle13','cle14','cle15','cle16','cle17','cle18','cle19','cle20','cle21','cle22','cle23','cle24','cle25','cle26','cle27','cle28','cle29','cle30','cle31','cle32','cle33','cle34','cle35','cle36','cle37','cle38','cle39','cle40','cle41','cle42','cle43','cle44','cle45','cle46','cle47','cle48','cle49','cle50','cle51','cle52','cle53','cle54','cle55','cle56','cle57','cle58','cle59','cle60','cle61','cle62','cle63','cle64','cle65','cle66','cle67','cle68','cle69','cle70','cle71','cle72','cle73','cle74','cle75','cle76','cle77','cle78','cle79','cle80','cle81','cle82','cle83','cle84','cle85','cle86','cle87','cle88','cle89','cle90','cle91','cle92','cle93','cle94','cle95','cle96','cle97','cle98','cle99','extra_attributes','creat_by','deleted_at','created_at','updated_at','identifiants_sadge',],
        'graphiques':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'groupedirections':['id','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'groupepermissions':['id','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'headselements':['id','cle','valeur','entreprise_id','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'historiquemodelslistings':['id','action','ancien','nouveau','modelisting_id','user_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'historiques':['id','type','cle','valeur','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'homes':['id','user','etat','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'homezones':['id','libelle','type','zone_id','modelslisting_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','modelslisting','effectifsjour','presentsjour','effectifsnuit','presentsnuit','identifiants_sadge',],
        'horaireagents':['id','horaire_id','user_id','lun','mar','mer','jeu','ven','sam','dim','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by','typesagents',],
        'horaires':['id','libelle','debut','fin','tolerance','type','extra_attributes','created_at','updated_at','parent','parentId','vol_horaire_min','nmb_pointage_min','deleted_at','identifiants_sadge','creat_by','poste_id',],
        'horairesglobals':['id','libelle','horaire',],
        'horairesglobalspostes':['id','libelle','horaire',],
        'horairesglobalstaches':['id','libelle','horaire',],
        'horairestypespostes':['id','libelle','debut','fin','typesposte_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','ordre',],
        'horairestypessites':['id','libelle','debut','fin','typessite_id','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'identifications':['id','user_id','carte_id','date_debut','date_fin','statuts','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'imports':['id','type','liaisons','identifiant','etats','creat_by','created_at','updated_at','extra_attributes','deleted_at','file','newtables','create','update','delete','valider','identifiants_sadge','description','typesposte_id','typeseffectif_id','typespointage_id','typespointages',],
        'interventiondetails':['id','interventionuser_id','jour','debut','fin','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'interventionimages':['id','intervention_id','path','type','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'interventions':['id','ref','agent','debut_prevu','debut_realise','fin_prevu','fin_realise','etats','extra_attributes','created_at','updated_at','site_id','site_libelle','client_id','client_libelle','deleted_at','identifiants_sadge','creat_by',],
        'interventionusers':['id','intervention_id','user_id','statut','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'jobs':['id','queue','payload','attempts','reserved_at','available_at','created_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'joursferies':['id','raison','debut','fin','etats','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'lignes':['id','ville_id','code','libelle','tarifs','deleted_at','creat_by','identifiants_sadge','extra_attributes','created_at','updated_at',],
        'lignesmoyenstransports':['id','moyenstransport_id','ligne_id','heure_debut','heure_fin','lun','mar','mer','jeu','ven','sam','dim','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'listesappels':['id','libelle','debut','fin','etats','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'listesappelsjours':['id','rand','jour01','jour02','jour03','jour04','jour05','jour06','jour07','jour08','jour09','jour10','jour11','jour12','jour13','jour14','jour15','jour16','jour17','jour18','jour19','jour20','jour21','jour22','jour23','jour24','jour25','jour26','jour27','jour28','jour29','jour30','jour31','tache01','tache02','tache03','tache04','tache05','tache06','tache07','tache08','tache09','tache10','tache11','tache12','tache13','tache14','tache15','tache16','tache17','tache18','tache19','tache20','tache21','tache22','tache23','tache24','tache25','tache26','tache27','tache28','tache29','tache30','tache31','listesappel_id','user_id','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'listesjours':['id','rand','jour','listesappel_id','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'listings':['id','date','id_user','name','nom','prenom','matricule','num_badge','actif_id','nationalite_id','contrat_id','direction_id','categorie_id','echelon_id','sexe_id','matrimoniale_id','poste_id','ville_id','zone_id','situation_id','balise_id','fonction_id','emp_code','online_id','type_id','faction_id','present','site_id','client_id','id_date','deleted_at',],
        'listingsetats':['id','listingsjour_id','user_id','present','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'listingsjours':['id','libelle','date','etats','extra_attributes','created_at','updated_at','user','deleted_at','identifiants_sadge','creat_by',],
        'logins':['id','email','password','etat','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'logs':['id','action','ip','details','navigateur','pays','ville','user_id','created_at','updated_at','deleted_at','extra_attributes','identifiants_sadge','creat_by',],
        'materielinterventiondetails':['id','materiel_id','materielintervention_id','quantite','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'materielinterventions':['id','intervention_id','type','libelle','date','created_at','updated_at','valider','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'materielprevisionnels':['id','materiel_id','chantier_id','quantite','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'materiels':['id','libelle','reference','description','quantite','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'matrices':['id','libelle','date_debut','date_fin','etats','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'matrimoniales':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'menus':['id','name','icon','slug','url','ordre','isSu','menu_id','entreprise_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'mesurespreventives':['id','libelle','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'model_has_permissions':['permission_id','model_type','model_id','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'model_has_roles':['role_id','model_type','model_id','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'modelslistings':['id','Libelle','extra_attributes','deleted_at','created_at','updated_at','postes','zone_id','faction','user_id','date_debut','min_partage','Generate','etats','user_id_2','user_id_3','user_id_4','identifiants_sadge','creat_by','typelistings','horaires','directions',],
        'moyenstransports':['id','code','libelle','typesmoyenstransport_id','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'nationalites':['id','libelle','code','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'oauth_access_tokens':['id','user_id','client_id','name','scopes','revoked','created_at','updated_at','expires_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'oauth_auth_codes':['id','user_id','client_id','scopes','revoked','expires_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'oauth_clients':['id','user_id','name','secret','provider','redirect','personal_access_client','password_client','revoked','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'oauth_personal_access_clients':['id','client_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'oauth_refresh_tokens':['id','access_token_id','revoked','expires_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'objectifs':['id','libelle','debut','fin','description','activite_id','user_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'onlines':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'passagesrondes':['id','heure_debut','heure_fin','lun','mar','mer','jeu','ven','sam','dim','site_id','creat_by','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','libelle',],
        'pastilles':['id','code','libelle','site_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'permissions':['id','name','guard_name','created_at','updated_at','deleted_at','extra_attributes','type','nom','visible','groupepermission_id','identifiants_sadge','creat_by',],
        'permissionsdetails':['id','action','table','creat_by','user_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge',],
        'perms':['id','permission_label','permission_nom','permission_id','updated_at','user_id','nom','prenom','type','deleted_at','created_at',],
        'pointages':['id','pointeuse','lieu','debut_prevu','fin_prevu','faction_horaire','debut_reel','debut_realise','fin_realise','volume_realise','emp_code','motif','volume_prevu','actif','est_valide','horaire_id','programme_id','tolerance','est_attendu','etats','user_id','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'pointeuses':['id','code','libelle','created_at','updated_at','nom_local','supervirzclient_id','code_teleric','postes','Taches','lun','mar','mer','jeu','ven','sam','dim','site_id','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'pointeusestransactions':['transactions_totals','transactions_heures','transactions_id','date','pointeuse',],
        'points':['id','libelle','longitude','latitude','ville_id','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'positions':['id','lat','lon','name','title','speed','icon_color','moyenstransportid','creat_by','extra_attributes','created_at','updated_at','deleted_at','date','tracername','traceruniqueid','sim','balise_id',],
        'postes':['id','code','libelle','nature','coordonnees','site_id','created_at','updated_at','jours','contratsclient_id','maxjours','maxnuits','NbrsJours','NbrsNuits','IsCouvert','pointeuses','Agentjour','Agentnuit','couvertAgentjour','couvertAgentnuit','type','extra_attributes','deleted_at','identifiants_sadge','creat_by','typeagents','typesposte_id','postesarticle_id',],
        'postesagents':['id','poste_id','user_id','faction','created_at','updated_at','extra_attributes','deleted_at','lun','mar','mer','jeu','ven','sam','dim','identifiants_sadge','creat_by',],
        'postesarticles':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'postesglobals':['id','libelle','code','site','zone',],
        'postespointeuses':['id','poste_id','pointeuse_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'presences':['id','raison','debut','fin','user_id','etats','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'prestations':['id','code','libelle','description','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'preuves':['id','programme_id','transaction_id','punch_time','type','role','etats','extra_attributes','created_at','updated_at','valide','remarque','deleted_at','identifiants_sadge','creat_by',],
        'processus':['id','libelle','description','valide_one','valide_two','work_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'programmations':['id','libelle','description','date_debut','date_fin','default_heure_debut','default_heure_fin','tache_id','user_id','statut','type','extra_attributes','created_at','updated_at','poste_id','faction','etats','valider1','valider2','postes','Allclients','AllDatesInRange','Presents','Abscents','Presentsid','Abscentsid','zone_id','user_id_2','user_id_3','user_id_4','min_pointage','deleted_at','identifiants_sadge','creat_by','valideur_1','valideur_2','typelistings','postesbaladeur','directions',],
        'programmationsdetails':['debut','fin','users','programmation_id',],
        'programmationsrondes':['id','libelle','description','date_debut','date_fin','default_heure_debut','default_heure_fin','user_id','statut','type','postesbaladeur','valider1','zone_id','valider2','poste_id','etats','postes','min_pointage','Allclients','AllDatesInRange','Presents','Abscents','Presentsid','Abscentsid','user_id_2','user_id_3','user_id_4','valideur_1','valideur_2','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'programmationsusers':['id','user_id','programmation_id','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'programmes':['id','date','debut_prevu','fin_prevu','debut_reel','debut_realise','fin_realise','volume_horaire','hs_base','hs_hors_faction','hs_in_faction','programmationsuser_id','horaire_id','etats','totalReel','totalFictif','extra_attributes','created_at','updated_at','poste_id','remplacant','type','week','user','DayStatut','Remplacantuser','PresencesDeclarer','AbscencesDeclarer','EtatsDeclarer','Totalpresent','J1','J2','J3','J4','J5','J6','J7','J8','J9','J10','J11','J12','J13','J14','J15','J16','J17','J18','J19','J20','J21','J22','J23','J24','J25','J26','J27','J28','J29','J30','J31','deja_annaliser','pointages_rattacher_auto','pointages_rattacher_manuel','pointages_debut_auto','pointages_debut_manuel','pointages_fin_auto','pointages_fin_manuel','presence_declarer_auto','presence_declarer_manuel','deleted_at','identifiants_sadge','creat_by','programmation_id','user_id','qualification_horaire','fin_reel','typesheure_id',],
        'programmesrondes':['id','date','debut_prevu','fin_prevu','debut_reel','debut_realise','fin_realise','volume_horaire','hs_base','hs_hors_faction','hs_in_faction','programmationsuser_id','horaire_id','etats','totalReel','totalFictif','poste_id','remplacant','type','week','user','DayStatut','Remplacantuser','PresencesDeclarer','AbscencesDeclarer','EtatsDeclarer','Totalpresent','J1','J2','J3','J4','J5','J6','J7','J8','J9','J10','J11','J12','J13','J14','J15','J16','J17','J18','J19','J20','J21','J22','J23','J24','J25','J26','J27','J28','J29','J30','J31','deja_annaliser','pointages_rattacher_auto','pointages_rattacher_manuel','pointages_debut_auto','pointages_debut_manuel','pointages_fin_auto','pointages_fin_manuel','presence_declarer_auto','presence_declarer_manuel','programmationsronde_id','user_id','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'projets':['id','libelle','descriptions','debut_previsionnel','fin_previsionnel','debut_reel','fin_reel','creat_by','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge',],
        'provinces':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'rapportpostes':['id','total','date','poste_id','created_at','updated_at','deleted_at','extra_attributes','identifiants_sadge','creat_by',],
        'rapports':['id','mois','poste_id','ville_id','zone_id','fonction_id','type_id','faction_id','site_id','client_id','day_01','day_02','day_03','day_04','day_05','day_06','day_07','day_08','day_09','day_10','day_11','day_12','day_13','day_14','day_15','day_16','day_17','day_18','day_19','day_20','day_21','day_22','day_23','day_24','day_25','day_26','day_27','day_28','day_29','day_30','day_31','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'recuperes':['id','libelle','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'ressources':['id','type','cle','valeur','activite_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'role_has_permissions':['id','permission_id','role_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by','canCreate','canUpdate','canDelete',],
        'roles':['id','name','guard_name','created_at','updated_at','deleted_at','extra_attributes','type','identifiants_sadge','creat_by',],
        'services':['id','code','libelle','extra_attributes','created_at','updated_at','direction_id','deleted_at','identifiants_sadge','creat_by',],
        'sexes':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'sites':['id','libelle','client_id','zone_id','created_at','updated_at','pointeuse_id','NbrsJours','NbrsNuits','type','extra_attributes','deleted_at','identifiants_sadge','creat_by','pastille','typessite_id','date_debut','date_fin',],
        'sitesglobals':['id','created_at','deleted_at','libelle','Selectlabel',],
        'sitespointeuses':['id','site_id','pointeuse_id','retirer','creat_by','extra_attributes','created_at','updated_at','deleted_at','debut','fin',],
        'sitessdeplacements':['id','deplacement_id','site_id','durees','creat_by','extra_attributes','created_at','updated_at','deleted_at','date',],
        'situations':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'soldables':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'statszones':['id','nom1','modelslistingnuit1_id','modelslistingjour1_id','nom2','modelslistingnuit2_id','modelslistingjour2_id','nom3','modelslistingnuit3_id','modelslistingjour3_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','user_id','modelslistingnuit1','modelslistingnuit2','modelslistingnuit3','modelslistingjour1','modelslistingjour2','modelslistingjour3','identifiants_sadge',],
        'supervirzclients':['id','nom','domaine','path','created_at','updated_at','db_connection','db_host','db_port','db_database','db_username','db_password','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'supervirzclientshides':['id','libelle','supervirzclient_id','extra_attributes','deleted_at','created_at','updated_at','identifiants_sadge','creat_by',],
        'surveillances':['id','action','entite','entite_cle','ancien','nouveau','ip','details','navigateur','pays','ville','user_id','id_base','created_at','updated_at','deleted_at','extra_attributes','identifiants_sadge','creat_by',],
        'switchsusers':['id','old_type','new_type','action','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'taches':['id','typestache_id','libelle','extra_attributes','created_at','updated_at','pastille','Pointeuses','deleted_at','identifiants_sadge','creat_by','site_id','ville_id','jours','code','maxjours','maxnuits','NbrsJours','NbrsNuits','IsCouvert','Agentjour','Agentnuit','couvertAgentjour','couvertAgentnuit',],
        'tachespointeuses':['id','tache_id','pointeuse_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'terminals':['id','code','adresse_mac','etat','alimentation','reseau','voiture_id','creat_by','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge',],
        'trackings':['id','balise_id','moyenstransport_id','date_debut','date_fin','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'traitements':['id','libelle','date','etat_depart','etat_arrive','transaction_id','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'trajets':['id','ligne_id','distance','deleted_at','creat_by','identifiants_sadge','extra_attributes','created_at','updated_at','site_id','durees','ordre',],
        'transactionhistoriques':['id','depuis','transaction_id','created_at','updated_at','deleted_at','extra_attributes','identifiants_sadge','creat_by',],
        'transactions':['id','bio_id','area_alias','first_name','last_name','card_no','terminal_alias','emp_code','punch_date','punch_time','nom','prenom','matricule','echelon_id','sexe_id','matrimoniale_id','poste_id','ville_id','zone_id','situation_id','balise_id','fonction_id','online_id','faction_id','pointeuse_id','site_id','client_id','extra_attributes','created_at','updated_at','annuler','type','traiter','pointeusepostes','verification','rechercheetape','tache','poste','TachesPotentiels','PostesPotentiels','TotalPostes','TotalPostescouvert','TotalPostesnoncouvert','TotalPostessouscouvert','heure','deleted_at','identifiants_sadge','creat_by','etats','identification_id','controlleursacce_id','carte_id','cout','ligne_id'],
        'transactionsdetails':['id','parent','parentId','transaction_id','created_at','updated_at','creat_by','extra_attributes','deleted_at','identifiants_sadge',],
        'transactionspostessyntheses':['transactions_totals','transactions_id','transactions_matricule','transactions_heures','date','poste_id',],
        'transactionspostessynthesesvacations':['transactions_totals','poste_id','transactions_id','transactions_heures','date',],
        'transactionssyntheses':['transactions_totals','transactions_heures','transactions_id','matricule','date',],
        'transactionsulterieurs':['id','date','transaction_id','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'transactionsuserssyntheses':['transactions_totals','transactions_id','transactions_heures','matricule','date',],
        'transactionsuserssynthesesvacations':['transactions_totals','matricule','transactions_id','transactions_heures','date',],
        'transporteurs':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'transporteurstrajets':['id','transporteur_id','trajet_id','montant','debut','fin','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'travailleurs':['id','horaire_id','user_id','lun','mar','mer','jeu','ven','sam','dim','created_at','updated_at','extra_attributes','deleted_at','tache_id','identifiants_sadge','creat_by',],
        'typeinterventions':['id','libelle','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'types':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'typesabscences':['id','libelle','soldable_id','variable_id','nombrejours','etats','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'typesagentshoraires':['id','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'typeseffectifs':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','canCreate','canUpdate','canDelete','champHide',],
        'typesheures':['id','code','libelle','description','creat_by','extra_attributes','created_at','updated_at','deleted_at','type',],
        'typesmoyenstransports':['id','code','libelle','canCreate','canUpdate','canDelete','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'typespointages':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'typespostes':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','canCreate','canUpdate','canDelete','maxagent',],
        'typessites':['id','code','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at','canCreate','canUpdate','canDelete',],
        'typestaches':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'typesventilations':['id','libelle','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'userbadges':['id','user_id','num_badge','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'users':['id','name','nom','prenom','matricule','num_badge','date_naissance','num_cnss','num_cnamgs','telephone1','telephone2','photo','date_embauche','download_date','actif_id','nationalite_id','contrat_id','direction_id','categorie_id','echelon_id','sexe_id','matrimoniale_id','poste_id','ville_id','zone_id','site_id','situation_id','balise_id','fonction_id','user_id','email','email_verified_at','password','emp_code','nombre_enfant','num_dossier','online_id','type','faction_id','remember_token','extra_attributes','created_at','updated_at','role_id','deleted_at','identifiants_sadge','creat_by','typeseffectif_id','postes',],
        'usersgraphiques':['id','user_id','graphique_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'userstypespostes':['id','user_id','typesposte_id','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'userszones':['id','user_id','zone_id','created_at','updated_at','deleted_at','extra_attributes','identifiants_sadge','creat_by',],
        'vacationspostes':['id','total','date','poste_id','created_at','updated_at','deleted_at','extra_attributes','identifiants_sadge','creat_by',],
        'validations':['id','libelle','users','modelslisting_id','creat_by','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','nmbvalideurs',],
        'variables':['id','libelle','code','remember_token','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'vehicules':['id','code','type','marque','modele','generation','serie','valeur','moteur','poids','creat_by','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge',],
        'ventilations':['id','user_id','semaine','dimanche_date','lundi_date','mardi_date','mercredi_date','jeudi_date','vendredi_date','samedi_date','dimanche_horaire','lundi_horaire','mardi_horaire','mercredi_horaire','jeudi_horaire','vendredi_horaire','samedi_horaire','dimanche','lundi','mardi','mercredi','jeudi','vendredi','samedi','dimanche_pointage','lundi_pointage','mardi_pointage','mercredi_pointage','jeudi_pointage','vendredi_pointage','samedi_pointage','dimanche_collecter','lundi_collecter','mardi_collecter','mercredi_collecter','jeudi_collecter','vendredi_collecter','samedi_collecter','dimanche_depassement','lundi_depassement','mardi_depassement','mercredi_depassement','jeudi_depassement','vendredi_depassement','samedi_depassement','dimanche_programmer','lundi_programmer','mardi_programmer','mercredi_programmer','jeudi_programmer','vendredi_programmer','samedi_programmer','dimanche_retard','lundi_retard','mardi_retard','mercredi_retard','jeudi_retard','vendredi_retard','samedi_retard','programmation_id','total_programmer','total_colecter','total_depassement','hs15','hs26','hs55','hs30','hs60','hs115','hs130','total','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'villes':['id','libelle','code','extra_attributes','created_at','updated_at','deleted_at','identifiants_sadge','creat_by',],
        'villeszones':['id','ville_id','zone_id','creat_by','extra_attributes','created_at','updated_at','deleted_at',],
        'voitures':['id','code','libelle','plaque','capacite','deleted_at','creat_by','identifiants_sadge','extra_attributes','created_at','updated_at',],
        'websockets_statistics_entries':['id','app_id','peak_connection_count','websocket_message_count','api_message_count','created_at','updated_at','extra_attributes','deleted_at','identifiants_sadge','creat_by',],
        'works':['id','libelle','activite_id','user_id','created_at','updated_at','extra_attributes','deleted_at','debut','fin','groupe','identifiants_sadge','creat_by',],
        'zones':['id','code','libelle','province_id','created_at','updated_at','total_titulaires_therorique','total_titulaires_reel_jour','total_titulaires_reel_nuit','total_present_jour','total_present_nuit','ordre','extra_attributes','deleted_at','identifiants_sadge','creat_by','ville_id',],
      ...
      String domaine = "";
      $dbDto.result.forEach((element) {
        if(element['cle']=="tableSynchro"){
          tableSyncro = element['valeur'].toString().split(",");
        }
        if(element['cle']=="domaine"){
          domaine = element['valeur'].toString();
        }
      });
      ...
      print($dbDto.result);
      int page = 1;
      tableSyncro.forEach((element) async {
        bool $allGetUpdate=false;
        while(!$allGetUpdate){
          Map aggridParam;
          String lastUpdateDate = '1000-01-01';
          DatabaseDto $lastUpdateDto = DatabaseManager.getDto();
          $lastUpdateDto=DatabaseManager.setTable($lastUpdateDto, element);
          $lastUpdateDto=DatabaseManager.addLimit($lastUpdateDto, 1);
          $lastUpdateDto=DatabaseManager.addOrder($lastUpdateDto, 'updated_at','DESC');
          $lastUpdateDto=await DatabaseManager.read($lastUpdateDto, ['*']);
          $lastUpdateDto.result.forEach((element) {
            lastUpdateDate = element['updated_at'].toString();
          });
          aggridParam=new Server()
              .table(element)
              .orderBy('updated_at', 'ASC')
              .max(100)
              .where('updated_at', lastUpdateDate, 'superieur').setPage(page).getParams();

          print('Voici le dernier element present chez nous');
          print(element);
          ...
          if($apiDto.status==200 ){
            ...
            print($apiDto.result);
            List<Map> newDatas = [];
            $apiDto.result['rowData'].forEach((donne) {
              Map newData={};
              champs[element]?.forEach((column) {
                newData[column] = donne[column];
              });
              newDatas.add(newData);
            });
            for(int i=0;i<newDatas.length;i++){
              Map data=newDatas[i];
              DatabaseDto $syncSql = DatabaseManager.getDto();
              $syncSql=DatabaseManager.setTable($syncSql, element);
              $syncSql=DatabaseManager.addWhere($syncSql, 'id','=',data['id']);
              await DatabaseManager.create($syncSql,data);
              await DatabaseManager.update($syncSql,data);
            }
            if($apiDto.result['rowCount']==0){

              print(element);
              print('on arrete de recuperer les update');
              $allGetUpdate=true;
            }else{

              print(element);
              print('on continue a recuperer les update');
            }
        ...
        bool $allGetCreate=false;
        while(!$allGetCreate){
          Map aggridParam;
          String lastCreateDate = '1000-01-01';
          DatabaseDto $lastCreateDto = DatabaseManager.getDto();
          $lastCreateDto=DatabaseManager.setTable($lastCreateDto, element);
          $lastCreateDto=DatabaseManager.addLimit($lastCreateDto, 1);
          $lastCreateDto=DatabaseManager.addOrder($lastCreateDto, 'created_at','DESC');
          $lastCreateDto=await DatabaseManager.read($lastCreateDto, ['*']);
          $lastCreateDto.result.forEach((element) {
            lastCreateDate = element['created_at'].toString();
          });
          aggridParam=new Server()
              .table(element)
              .orderBy('created_at', 'ASC')
              .max(100)
              .where('created_at', lastCreateDate, 'superieur').setPage(page).getParams();
          ...
          if($apiDto.result['rowCount']==0){
            ...
            print(element);
            print('on arrete de recuperer les create');
            $allGetCreate=true;
          }else{
            print(element);
            print('on continue a recuperer les create');
          }
          $allGetCreate=true;
        }
      });
      for (int k=0;k<transactions.length;k++){
        ...
        if($apiDto.status==200){
          ...
          print($apiDto.result);
          DatabaseDto $syncTransactionSql = DatabaseManager.getDto();
          $syncTransactionSql=DatabaseManager.setTable($syncTransactionSql, 'transactions');
          $syncTransactionSql=DatabaseManager.addWhere($syncTransactionSql, 'id','=',id);
          await DatabaseManager.delete($syncTransactionSql);
        }else{
